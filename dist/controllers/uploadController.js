...
const uploadController = async (req, res) => {
    try {
            ...
            try {
                ...
                for (const row of rows) {
                    if (fileType === "inventory") {
                        const productName = String(getValue(row, ["productName", "product_name", "name"]) ||
                            "Unknown Product");
                        ...
                        const orderAtLeast = toNumberOrNull(getValue(row, ["orderAtLeast", "order_at_least"]));
                        const avgDailyDemand = toNumberOrNull(getValue(row, ["avg_daily_demand", "avgDailyDemand"]));
                        ...
                        await prisma_1.prisma.inventory.create({
                            data: {
                                ...
                                orderAtLeast: orderAtLeast === null ? undefined : Math.round(orderAtLeast),
                                avgDailyDemand: avgDailyDemand === null ? undefined : avgDailyDemand,
                                stockMonth: stockMonth ?? undefined,
                                userId,
                            },
                        });
                        console.log(`✅ Saved perfectly: ${productName} with stock: ${stock}`);
                    }
                    else {
                        const itemName = getValue(row, ["item_name", "itemName"]) || "Unknown Item";
                        ...
                        const transactionNum = getValue(row, ["transaction_number", "transactionNumber"]) ||
                            "N/A";
                        const dateValue = getValue(row, ["transaction_date", "transactionDate"]) || "";
                        const quantity = quantityValue ? Number(quantityValue) : 0;
                        const explicitTotalPrice = totalPriceValue === undefined ? null : Number(totalPriceValue);
                        const price = priceValue !== undefined
                            ? Number(priceValue)
                            : explicitTotalPrice !== null && quantity > 0
                                ? explicitTotalPrice / quantity
                                : 0;
                        const totalPrice = explicitTotalPrice !== null && !Number.isNaN(explicitTotalPrice)
                            ? explicitTotalPrice
                        ...
                        if (!transactionDate) {
                            return (0, helpers_1.sendJSON)(res, 400, {
                                message: "Invalid transaction date. Use transaction_date or transactionDate with a valid date.",
                                value: dateValue,
                            });
                        }
                        const itemYear = transactionDate.getFullYear();
                        const status = currentYear - itemYear >= 2 ? "archive" : "active";
        ...
        return (0, helpers_1.sendJSON)(res, 500, { message: "Server Error during upload." });
    }
};
